...
public interface RedisClient {
     ...
     *
     * @param key
     * @param seconds 保留多少秒的Key以供判断是否重复
     * @return
     */
     ...
     * 自增(如果没有找到key，会自动创建key并设置为1)
     *
     * @param key        如果是第1次创建这个key，则为1
     * @param expireTime 如果是第1次创建这个key，设置保持这个KV多少秒
     * @return 自增后的值
     */
     ...
     * @param key
     * @param value
     * @throws RedisSetException 设置错误时抛出异常
     */
     ...
     *
     * @param key
     * @param value 将value对象序列化后存储到redis中
     * @throws RedisSetException 设置错误时抛出异常
     */
     ...
     * 模糊查询
     *
     * @param key   模糊查询，可以用*和?作通配符
     * @param clazz Value的类
     * @return 如果找不到key，返回列表大小为0
     */
    <T> Set<T> listByWildcard(String key, Class<T> clazz);
     ...
     * @return 如果找不到key，返回列表大小为0
     */
    default Set<String> listByWildcard(final String key) {
